const { mockQdrantClient, mockCreateEmbedding } = vi.hoisted(() => ({
  mockQdrantClient: {
...
describe("upsertAgenda", () => {
  ...
  it("deletes existing agenda and upserts new one", async () => {
    ...
    expect(mockCreateEmbedding).toHaveBeenCalledWith("agenda chunk");
